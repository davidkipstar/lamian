...
def find_price(orderbook, ob_th, tsize, previous_order = None, previous_amount = None, previous_price = None, minimum_liquidity=1):
    ...
    tsize = Decimal(tsize).quantize(satoshi) if isinstance(tsize, int) else Decimal(tsize['amount']).quantize(satoshi) # Can be passed as integer if the balance is 0 when starting Analyst
